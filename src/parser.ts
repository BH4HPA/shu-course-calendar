 ...
 */
export function scheduleHtmlParser(): ParserResult {
      ...
      if (colContents.length >= Math.max(...indexes)) {
        courseInfos.push(
          ..._toCourseInfos(
            ...
            _getPeriods(
              colContents[indexes[2]],
              indexes[4] >= 0 ? colContents[indexes[4]] : '',
              isShortTerm
            )
          )
        );
...
function _tryGetIndexes(tr: Cheerio): number[] | undefined {
  ...
  const indexes = [
    ...
    '上课教室',
    '上课日期',
  ].map((value) => colContents.indexOf(value) - 1);
...
export function _toCourseInfos(
  name: string,
  teacher: string,
  position: string,
  periods: _ClassPeriod[]
): CourseInfo[] {
...
export function _getPeriods(
  ...
  const globalWeekPatterns = _getWeekPatternsFromDateText(
    rawDateText,
    isShortTerm ? 4 : 10
  );
  rawTimeText = rawTimeText.replace(/、/gi, ',');
  const weekRegExp = /\((?<week>[^(]*?\d+(?:\s*[-,]\s*\d+)?周[^(]*?)\)/gi;
  let weekRes: RegExpExecArray | null = null;
  let startIndex = 0;
  while (
    (weekRes = weekRegExp.exec(rawTimeText)) != null &&
    weekRes.groups != null
  ) {
    const {
      groups: { week },
    } = weekRes;
    const classPeriods = _getClassPeriodsFromTimeTextPart(
      rawTimeText.slice(startIndex, weekRegExp.lastIndex)
    );
  ...
  periods.push(
    ..._getClassPeriodsFromTimeTextPart(rawTimeText.slice(startIndex))
  );
...
function _getWeekPatternsFromDateText(
  rawDateText: string,
  weeksNum: number
): number[] {
  const datePattern =
    /(?<mFrom>\d+)月(?<dFrom>\d+)日?至(?<mTo>\d+)月(?<dTo>\d+)日?/i;
  const dateRes = datePattern.exec(rawDateText);
  if (dateRes != null && dateRes.groups != null) {
    const {
      groups: { mFrom, dFrom, mTo, dTo },
    } = dateRes;
    const [monthFrom, dateFrom, monthTo, dateTo] = [mFrom, dFrom, mTo, dTo].map(
    ...
    if (
      to >= from &&
      from >= new Date(2020, 8, 7) &&
      to < new Date(2020, 10, 30)
    ) {
      const base = new Date(2020, 8, 7);
      const fromDays = Math.round(
        (from.getTime() - base.getTime()) / 1000 / 3600 / 24
      );
      const toDays = Math.round(
        (to.getTime() - base.getTime()) / 1000 / 3600 / 24
      );
...
function _getWeekPatternFromWeekText(weekText: string): number {
  let result = 0x0;
  const weekRegExp =
    /(?<num1>\d+)(?:\s*(?<connector>[-,])\s*(?<num2>\d+))?周/gi;
  let weekRes: RegExpExecArray | null;
  while (
    (weekRes = weekRegExp.exec(weekText)) != null &&
    weekRes.groups != null
  ) {
    const {
      groups: { num1, connector, num2 },
    } = weekRes;
...
function _getClassPeriodsFromTimeTextPart(
  timeTextPart: string
): _ClassPeriod[] {
  const periods: _ClassPeriod[] = [];
  const classTimeRxp =
    /(?<day>[一二三四五])(?<from>\d+)(?:-(?<to>\d+))?(?<oddEven>[单双])?/gi;
  let classTimeRes: RegExpExecArray | null = null;
  while (
    (classTimeRes = classTimeRxp.exec(timeTextPart)) != null &&
    classTimeRes.groups != null
  ) {
    const {
      groups: { day, from, to, oddEven },
    } = classTimeRes;
    periods.push({
      weekPattern:
        oddEven === '单' ? 0x555555 : oddEven === '双' ? 0xaaaaaa : 0xffffff,
...
export function _getSectionTimes(): SectionTime[] {
  return [
    ['8:00', '8:45'],
    ['8:55', '9:40'],
    ['10:00', '10:45'],
